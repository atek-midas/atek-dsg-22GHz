...
#define PLL_DEN 1000 
#define REFERANS_FREKANS 40000000
#define OSC_2X 2  
#define MULT 1
#define PLL_R_PRE 1
#define R_BOLUCU 1
  ...
  Serial.println(buf);
	Serial.println("\r\n");
	   ...
	   uint8_t cikis_bolucu = 1;
	    ...
	    double PFD_FREKANSI = (REFERANS_FREKANS * OSC_2X * MULT) / (PLL_R_PRE * R_BOLUCU);
	    ...
	    uint32_t reg_R_DIV = ((R_BOLUCU & 0xFF) | ((cikis_bolucu - 1) << 6)) | 0x1000;


      PLL_write(0x2B, reg_NUM2);
      uint16_t read_val = PLL_read(0x2B);
