  ...
  document.getElementById('setFrequency').addEventListener('click', () => {
    ...
    fetch('/setFrequency', {
      ...
      .then(data => {
        ...
        setTimeout(() => {
          button.style.backgroundColor = originalColor; // Restore original color
...
document.getElementById('save-credentials').addEventListener('click', async () => {
  ...
  const originalColor = button.style.backgroundColor; // originalColor'u burada tanımlayın
...
document.getElementById('clear-credentials').addEventListener('click', async () => {
  ...
  // RF Output Toggle
  document.getElementById('toggleRFOutput').addEventListener('click', () => {
    const button = document.getElementById('toggleRFOutput');
    const originalColor = button.style.backgroundColor;

    fetch('/toggleRFOutput', { method: 'POST' })
      .then(response => response.text())
      .then(data => {
        button.textContent = data === "true" ? "Turn Off" : "Turn On";
        button.style.backgroundColor = "green"; // Change color to green
        setTimeout(() => {
          button.style.backgroundColor = originalColor; // Restore original color
        }, 1000); // Restore after 1 second
...
void handleSetFrequency() {
    ...
    SetFreqOnMainMenu(currentFrequency); // Update display
    SetFreqUnitOnMainMenu(currentFreqUnit); 
    ...
    {
          // Hata mesajı gönder
      if (tempFrequency < MIN_FREQ || tempFrequency > 22600000000) {
...
//}
  ...
  SetAmpOnMainMenu(currentAmplitude); // Update display
  drawMainMenu();
  updateAmpArea();
  //saveRFSettings(); // Save to Preferences should be called on Demand
