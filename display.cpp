...
void setupTDisplayS3() {
  ...
  //oTouchController.setInvertY(true, 170);
...
*/
 ...
 bool checkenteredFreqValue() {
    ...
    if (freqValue < MIN_FREQ || freqValue > MAX_FREQ) {
      ...
      CurrentLockStatus = Lmx2820SetFreqinMHz(freqValue / 1000000.0);
      return true;
    }
}
...
void SetLock(bool value)
{
  ...
  if (value)
  {
    tft.pushImage(27, 151, 18, 18, (uint16_t*)Locked);
  }
  else
  {
    tft.pushImage(27, 151, 18, 18, (uint16_t*)UnLocked); 
  }
}
...
void SetTemp(const char* text)
{
  ...
  tft.fillRect(60, 151, 32, 18, tft.color565(51, 51, 51));
  tft.setTextColor(TFT_WHITE);
  tft.setFreeFont(&FreeSans12pt7b);
  tft.setTextSize(1);
  tft.setCursor(60, 151, 2);
